...
export default function TimestampConverter() {
  ...
  const handleTimestampChange = (val: string) => {
    ...
    if (!isNaN(ts)) {
      ...
      setDateString(t('invalidTimestamp'));
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="bg-blue-50 p-4 rounded-lg mb-8 text-center">
        <p className="text-sm text-blue-600 font-medium">{t('current')}</p>
        <p className="text-4xl font-mono font-bold text-blue-800 mt-2">{currentTimestamp}</p>
      </div>

      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">{t('labelTimestamp')}</label>
